...
export function registerEpicTools(server: McpServer, client: GitLabClient): void {
  server.registerTool("list_epics", {
    description:
      "Lister les epics du groupe GitLab. Filtrer par etat, recherche textuelle ou labels.",
    inputSchema: {
    ...
    try {
      const epics = await client.listEpics(args);
  ...
  server.registerTool("get_epic", {
    ...
    try {
      const epic = await client.getEpic(args.epic_iid);
  ...
  server.registerTool("create_epic", {
    description: "Creer un nouvel epic dans le groupe GitLab.",
    inputSchema: {
    ...
    try {
      const epic = await client.createEpic(args);
      return {
  ...
  server.registerTool("update_epic", {
    ...
    try {
      const { epic_iid, ...data } = args;
      const epic = await client.updateEpic(epic_iid, data);
      return {
  ...
  server.registerTool("close_epic", {
    ...
    try {
      const epic = await client.closeEpic(args.epic_iid);
      return {
  ...
  server.registerTool("list_epic_issues", {
    ...
    try {
      const issues = await client.listEpicIssues(args.epic_iid);
      if (issues.length === 0) {
  ...
  server.registerTool("add_issue_to_epic", {
    ...
    try {
      await client.addIssueToEpic(args.epic_iid, args.issue_id);
      return {
